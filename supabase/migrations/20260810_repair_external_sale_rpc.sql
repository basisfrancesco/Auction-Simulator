-- Ripristina la RPC per la vendita esterna e forza PostgREST a rilevarla.
-- La migrazione è idempotente e può essere eseguita anche se la funzione esiste già.
create or replace function public.sell_car_externally(
  p_car_id uuid,
  p_seller_name text,
  p_price numeric
)
...
as $$
declare
  c public.garage_cars%rowtype;
begin
  if auth.uid() is null then raise exception 'Sessione non valida'; end if;
  if p_price < 100 or p_price > 100000000 then
    raise exception 'Il prezzo deve essere compreso tra 100 € e 100.000.000 €';
  end if;
  ...
  update public.garage_cars
  set sold_at = clock_timestamp(), sale_price = p_price
  where id = p_car_id;
...
-- Supabase/PostgREST aggiorna immediatamente la cache delle funzioni RPC.
