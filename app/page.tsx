...
export default function Home() {
  ...
  const sellCarExternally = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (!user || !externalSaleCarId || marketPending) return;
    const price = Number(new FormData(event.currentTarget).get("externalSalePrice"));
    if (price < 100) return;
    setMarketPending(externalSaleCarId);
    const result = await supabase.rpc("sell_car_externally", { p_car_id: externalSaleCarId, p_seller_name: user.name, p_price: price });
    setMarketPending(null);
    if (result.error) { setConnectionError(result.error.message.replace("P0001: ", "")); return; }
    setExternalSaleCarId(null); setConnectionError(""); await loadParticipantProfile(user.name);
  };
  ...
  return <main className={`dashboard ${focused && !isAdmin ? "lobby-active" : ""} ${garageOpen || marketOpen ? "garage-active" : ""}`}>
    ...
    {externalSaleCarId && <div className="modal-backdrop" onMouseDown={() => setExternalSaleCarId(null)}><section className="modal compact-modal" onMouseDown={(event) => event.stopPropagation()}><button className="close" onClick={() => setExternalSaleCarId(null)}>×</button><div className="eyebrow"><i /> VENDITA ESTERNA</div><h2>Conferma la vendita.</h2><p>Stai vendendo <b>{garage.find((car) => car.id === externalSaleCarId)?.vehicle}</b> fuori dalla piattaforma. L’auto uscirà dal garage e la cifra verrà accreditata al saldo.</p><form onSubmit={sellCarExternally}><label>Prezzo concordato (€)<input name="externalSalePrice" type="number" min="100" max="100000000" step="50" placeholder="es. 45.000" autoFocus required /></label><div className="confirmation-actions"><button type="button" onClick={() => setExternalSaleCarId(null)}>ANNULLA</button><button className="primary" type="submit" disabled={marketPending === externalSaleCarId}>{marketPending === externalSaleCarId ? "VENDITA…" : "CONFERMA VENDITA"}</button></div></form></section></div>}
